<template>
  <v-container>
    <v-card>
      ...
      >
        <v-card-title>{{ menu.text }} {{ menu.price }} bath</v-card-title>
      </v-img>
      <v-card-actions>
      ...
      <v-card-text class="text-center">
        <strong class="display-1">price = {{ quantity * menu.price }}</strong>
      </v-card-text>
      <v-divider></v-divider>
      <v-card-actions>
        <v-btn dark color="pink" @click="addOrder">ເພີ່ມລົງໃນກະຕ່າ</v-btn>
        <v-spacer></v-spacer>
        <v-btn @click="$router.go(-1)">ຍົກເລີກ</v-btn>
      </v-card-actions>
    </v-card>
  </v-container>
</template>
